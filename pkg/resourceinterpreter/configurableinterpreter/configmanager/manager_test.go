...
func Test_interpreterConfigManager(t *testing.T) {
	...
	m := NewInterpreterConfigManager(informer)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := m.IsEnabled(tt.args.objGVK, tt.args.operation); got != tt.want {
...
func Test_interpreterConfigManager_Empty(t *testing.T) {
	m := &interpreterConfigManager{
		configuration: &atomic.Value{},
	}

	got := m.IsEnabled(appsv1.SchemeGroupVersion.WithKind("Deployment"), configv1alpha1.InterpreterOperationInterpretHealth)
	if got {
