...
func Test_initVM(t *testing.T) {
	script := "function test() return 1 end"
	timeoutScript := `
function timeout()
	while true do end
end
`
	...
	tests := []struct {
	...
	}{
		{
			...
			vm:      makeTestVM(),
			wantErr: false,
			want:    wants(wantInt32(1)),
		},
		{
			name: "vm stack is dirty",
			vm: func() *lua.LState {
				vm := makeTestVM()
				vm.Push(lua.LNumber(2))
				return vm
			}(),
			wantErr: false,
			want:    wants(wantInt32(1)),
		},
		{
			...
			vm: func() *lua.LState {
				...
				_, err := runScript(vm, timeoutScript, "timeout", 0, nil)
				if err == nil {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rets, err := runScript(tt.vm, script, "test", 1, nil)
			if err != nil {
				t.Fatalf("runScript() got error: %v", err)
			}
			if len(rets) != 1 {
				t.Fatalf("got rets %v, want %v", len(rets), 1)
			}
