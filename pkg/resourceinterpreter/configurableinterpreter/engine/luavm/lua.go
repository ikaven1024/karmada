...
func (v value) Into(obj interface{}) error {
	...
	//     {}
	// Luajson encodes it to `[]`. While go json fails to unmarshal it to
	// ReplicaRequirements object.
	//
	// So we replace the `[]` to `{}` before unmarshal to GO struct object.
	//
	// TODO: But how to handle empty struct in fields, such as:
